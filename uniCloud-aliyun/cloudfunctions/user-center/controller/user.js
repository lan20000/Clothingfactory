...
module.exports = {
	 ...
	 * 客户详情
	 */
	customerDetails: async function(e) {

	},
	/**
	 * @param {Object} 查询用户列表
	 */
	customerList: async function(e) {
		var {
			pageNumber,
			pageSize
		} = this.params;
	...
	/**
	 * @param {Object} 客户名称 查询用户
	 */
	queryUser: async function(e) {
		const {
			pageNumber,
			pageSize,
			username
	...
	save: async function(e) {
		...
		var sameNameUser = this.findFirst(await User.where({
			username
		}).limit(1).get());
		...
		var roleList = this.find(await Role.where({
			_id: dbCmd.in(data.roleIds)
		}).get());
